async function asyncForEach(array, callback) {
  ...
  async function getImages( currentIP, cameraType, index, cameraErrors, headNum, userName, password ) {
  ...
  if ( cameraType === "P" || cameraType === "PA" ) { 
    // adminUserForSnapShot = ["admin"];adminPassForSnapShot = ["timapp123"];
    urlImages = [
      "http://" + currentIP + "/jpeg",
    ];
  }
  adminUserForSnapShot = [userName];
  adminPassForSnapShot = [password];
