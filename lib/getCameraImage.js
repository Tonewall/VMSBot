...
function tryLoginForSnapShot(
  ...
  let urlSpec = "";
  if (cameraType === "A") urlSpec = "/media/cam0/still.jpg?res=max";
  ...
  return new Promise((resolve, reject) => {
    if (cameraErrors.picStatus === true) return resolve();
    ...
    request(
      ...
      function(error, response, body) {
        if (error && cameraErrors.picStatus === false) {
          ...
          cameraErrors.picDetails = "Unable to fetch image";
          ...
          if (
            ...
            cameraErrors.picDetails = "Unable to fetch picture";
            cameraErrors.picStatus = false;
          } else if (data.mimeType === "image/jpeg") {
            cameraErrors.picDetails = base64Str;
    ...
    cameraErrors.picStatus = false;
    cameraErrors.picDetails = `Unable to retrieve image. Error: ${error}`;
    return cameraErrors.picStatus === false
      ? resolve("no photo")
      : resolve("photo");
  });
...
async function getSnapShot(currentIP, cameraType, index, cameraErrors) {
